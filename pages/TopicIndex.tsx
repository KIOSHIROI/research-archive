...
import { useThemes, useContentByTheme } from '../contexts/ContentContext';
import { ContentCard } from '../components/ContentCard';
import { useLanguage } from '../contexts/LanguageContext';
import { FadeIn } from '../components/FadeIn';
import { SmoothText } from '../components/SmoothText';

const TopicSection: React.FC<{ theme: any, index: number }> = ({ theme, index }) => {
    const content = useContentByTheme(theme.id);
    ...
    return (
        <section id={theme.id} className="scroll-mt-32 relative pl-8 md:pl-12">
            {/* Timeline Node */}
    ...
    <div className="space-y-32">
      ...
      <div className="space-y-32 relative border-l border-stone-200 dark:border-ink-800 ml-4 md:ml-0 transition-colors">
        {themes.map((theme, index) => (
            <TopicSection key={theme.id} theme={theme} index={index} />
        ))}
      </div>
    </div>
  );
